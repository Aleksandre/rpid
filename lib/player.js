...
BasePlayer.prototype = {
	...
	init: function (options) {
		...
		this.allCommands = {
			...
			'mute': this.onMuted.bind(this),
			'increaseVol': this.onVolumeIncreased.bind(this),
			'decreaseVol': this.onVolumeDecreased.bind(this),
			'toggleSubtitle': this.onSubtitleToggled.bind(this),
		...
		this.playbackPosInSeconds = -1;
	},

	mute: function (cb) {
		this._sendCmdToThirdPartyPlayer('mute', cb);
	},

	onMuted: function () {
		if (this.volumeMute === -1) {
			this.volumeMute = this.volume;
			this.volume = 0;
		} else {
			this.volume = this.volumeMute;
			this.volumeMute = -1;
		}
	},

	increaseVol: function (cb) {
		this._sendCmdToThirdPartyPlayer('increaseVol', cb);
	},

	onVolumeIncreased: function () {
		winston.info("Player has increased volume");
		var newVolume = this.volume + this.volumeStep;
		if (newVolume > this.volumeMax) {
			newVolume = this.volumeMax;
		}
		this.volume = newVolume;
	},

	decreaseVol: function (cb) {
		this._sendCmdToThirdPartyPlayer('decreaseVol', cb);
	},

	onVolumeDecreased: function () {
		winston.info("Player has increased volume");
		var newVolume = this.volume + this.volumeStep;
		if (newVolume < 0) {
			newVolume = 0;
	...
	toggleSubtitle: function (cb) {
		this._sendCmdToThirdPartyPlayer('toggleSubtitle', cb);
	},

	onSubtitleToggled: function () {
		winston.info("Player has toggled subtitles");
		this.subtitle = !this.subtitle;
	},

	seek: function (timeInSeconds, cb) {
		this.stop((function () {
			this.playbackPosInSeconds = timeInSeconds;
			this.play(null, cb);
		}).bind(this));
	},
